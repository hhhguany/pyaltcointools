...
def encode_privkey(priv, formt, vbyte=0):
    ...
    if formt == 'decimal': return priv
        ...
        return bin_to_b58check(encode(priv, 256, 32), 45+int(vbyte))
    elif formt == 'wif_compressed':
        return bin_to_b58check(encode(priv, 256, 32)+'\x01', 45+int(vbyte))
    else: raise Exception("Invalid format!")
...
#【修改】
def privkey_to_address(priv, magicbyte=35):
...
#【修改】
def bin_to_b58check(inp, magicbyte=35):
...
#【修改】
def hex_to_b58check(inp, magicbyte=35):
...
#【修改】
def pubkey_to_address(pubkey, magicbyte=35):
